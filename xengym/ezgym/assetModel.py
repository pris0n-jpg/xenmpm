...
class ModelAsset(Asset):
    def __init__(
        ...
        self._asset = gym.load_asset(sim, asset_root, asset_file, asset_options)
    ...
    def getPose(self, idx=0) -> Matrix4x4:
        obj_handle = self.gym.find_actor_rigid_body_handle(Envs.envs[idx], self._actors[idx], "obj")
        obj_pose = self.gym.get_rigid_transform(Envs.envs[idx], obj_handle)
        return gymtf_to_matrix(obj_pose)
