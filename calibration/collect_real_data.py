...
class StageController:
    ...
    def _connect(self):
        try:
            ...
            if self.port in available_ports:
                ...
                logger.info(f"✓ 成功连接位移平台: {self.port}")
            else:
                for candidate in ["COM10", "COM11", "COM9", "COM7", "/dev/ttyUSB0", "/dev/ttyACM0"]:
                    if candidate in available_ports:
                        try:
                            self.serial = serial.Serial(candidate, self.baudrate, timeout=0.5)
                            self.port = candidate
                            logger.info(f"✓ 自动连接到: {candidate}")
                            break
                        except:
                            continue
            
            if not self.serial or not self.serial.is_open:
                raise RuntimeError("无法连接到位移平台串口")
            time.sleep(1)
        except Exception as e:
            logger.error(f"串口连接失败: {e}")
            raise
    
    def _send_command(self, cmd: str, wait_time: float = 0.5):
        if not self.serial or not self.serial.is_open:
        ...
        try:
            ...
            if self.serial.in_waiting:
                ...
                if response:
                    logger.debug(f"平台响应: {response}")
        except Exception as e:
            logger.error(f"命令发送失败: {e}")
            raise
    
    def move_z_absolute(self, z_mm: float):
        cmd = f"A{z_mm:.2f}"
        self._send_command(cmd, wait_time=0.5)
        self.current_z = z_mm
        logger.debug(f"Z轴移动到: {z_mm:.2f} mm")
    
    def move_z_relative(self, dz_mm: float):
        new_z = self.current_z + dz_mm
        self.move_z_absolute(new_z)
    ...
    def home(self):
        logger.info("执行归零...")
        self._send_command("A0", wait_time=1.0)
        self.current_z = 0.0
        self.current_x = 0.0
        self.current_y = 0.0
        logger.info("归零完成")
    ...
    def disconnect(self):
        if self.serial and self.serial.is_open:
            self.serial.close()
            logger.info("位移平台串口已断开")


class StageDataCollector():
    """位移平台数据采集器"""


    def __init__(self, object_name="circle_r4", config_file=None, storage_file=None, repeat_count=1, overwrite=False, port="COM10"):
        """
        初始化数据采集器
        ...
        # 初始化机器人
        self.stage = StageController(port=port)
        self.stage.home()
        time.sleep(1)
    ...
    def move_to_contact(self):
        ...
        safe_z = self.z_cont + self.safe_offset_mm if self.z_cont is not None else 10.0
        self.stage.move_z_absolute(safe_z)
        time.sleep(1)

        logger.info("开始缓慢下降检测接触...")
        step_size = 0.05
        is_contact = False
        ...
        self.stage.move_z_absolute(self.z_cont)
        time.sleep(0.5)
    ...
    def move_to_safe_position(self):
        """移动到安全位置"""
        safe_z = 10.0 + self.safe_offset_mm
        self.stage.move_z_absolute(safe_z)
        time.sleep(1)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="位移平台真实触觉数据采集（支持多次重复采集）")
    ...
    parser.add_argument("--port", type=str, default="COM10", help="位移平台串口（默认COM10）")
