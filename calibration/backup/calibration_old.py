    ...
    def _create_calibration_scene(self):
        ...
        for path in possible_paths:
            if path.exists():
                ...
                if stl_files:
                    object_files = [str(f) for f in stl_files[:3]]  # 使用前3个STL文件
                    break
...
class BayesianCalibration:
    ...
    def calculate_calibration_error(self, sim_data: Dict, real_data: Dict) -> float:
        ...
        for obj_name in real_data:
            if obj_name not in sim_data:
                continue
            
            for depth in real_data[obj_name]:
                if depth not in sim_data[obj_name]:
                    continue
                
                real_depth_data = real_data[obj_name][depth]
                sim_depth_data = sim_data[obj_name][depth]
                
                # 深度场误差 - 使用MSE和SSIM组合
                # if 'depth_field' in real_depth_data and 'depth_field' in sim_depth_data:
                #     real_field = np.array(real_depth_data['depth_field'])
                #     sim_field = np.array(sim_depth_data['depth_field'])
                    
                #     # 确保数据形状一致
                #     if real_field.shape != sim_field.shape:
                #         # 调整形状以匹配
                #         sim_field = np.resize(sim_field, real_field.shape)
                    
                #     # 计算均方根误差
                #     mse_error = np.sqrt(np.mean((real_field - sim_field) ** 2))
                    
                #     # 计算SSIM相似性 - 使用统一的标准化范围
                #     # 找到两个数据的全局最小值和最大值
                #     global_min = min(real_field.min(), sim_field.min())
                #     global_max = max(real_field.max(), sim_field.max())
                #     global_range = global_max - global_min + 1e-8
                    
                #     real_normalized = (real_field - global_min) / global_range
                #     sim_normalized = (sim_field - global_min) / global_range
                    
                #     try:
                #         # 计算SSIM，对于2D数据使用win_size=3
                #         if len(real_normalized.shape) == 2:
                #             ssim_value = ssim(real_normalized, sim_normalized, 
                #                             data_range=1.0, win_size=3, multichannel=False)
                #         elif len(real_normalized.shape) == 3:
                #             # 对于3D数据，逐个slice计算SSIM然后平均
                #             ssim_values = []
                #             for i in range(real_normalized.shape[2]):
                #                 ssim_slice = ssim(real_normalized[:,:,i], sim_normalized[:,:,i], 
                #                                 data_range=1.0, win_size=3, multichannel=False)
                #                 ssim_values.append(ssim_slice)
                #             ssim_value = np.mean(ssim_values)
                #         else:
                #             # 对于1D数据，使用相关系数作为替代
                #             ssim_value = np.corrcoef(real_normalized.flatten(), sim_normalized.flatten())[0,1]
                        
                #         # SSIM误差转换为误差形式 (1-SSIM，因为SSIM是相似性度量)
                #         ssim_error = 1.0 - abs(ssim_value)
                        
                #     except Exception as e:
                #         # 如果SSIM计算失败，回退到相关系数
                #         corr_coef = np.corrcoef(real_normalized.flatten(), sim_normalized.flatten())[0,1]
                #         ssim_error = 1.0 - abs(corr_coef)
                    
                #     # 组合MSE和SSIM误差 (权重可调)
                #     # depth_error = 0.7 * mse_error + 0.3 * ssim_error
                #     depth_error = ssim_error

                #     total_error += depth_error
                #     data_count += 1
                
                # Marker位移误差 
                if 'marker_displacement' in real_depth_data and 'marker_displacement' in sim_depth_data:
                    real_marker = np.array(real_depth_data['marker_displacement'])
                    sim_marker = np.array(sim_depth_data['marker_displacement'])
                    
                    # 确保数据形状一致
                    if real_marker.shape != sim_marker.shape:
                        sim_marker = np.resize(sim_marker, real_marker.shape)
                    
                    # 计算RMSE (均方根误差)
                    rmse_error = np.sqrt(np.mean((real_marker - sim_marker) ** 2))
                    # rmse_error = np.mean((real_marker - sim_marker) ** 2)
                    total_error += rmse_error*10
                    data_count += 1
                
                # 合力误差
                if 'force_xyz' in real_depth_data and 'force_xyz' in sim_depth_data:
                    real_force = np.array(real_depth_data['force_xyz'])
                    sim_force = np.array(sim_depth_data['force_xyz'])
                    
                    # rmse_error = np.mean((real_force - sim_force) ** 2)
                    rmse_error = np.sqrt(np.mean((real_force - sim_force) ** 2))
                    total_error += rmse_error/3
                    data_count += 1
    ...
    def save_results(self, results: Dict, file_path: Union[str, Path], E_true: Optional[float] = None, nu_true: Optional[float] = None, coef_true: Optional[float] = None):
        """保存标定结果"""
        file_path = Path(file_path)/"results"
...
def main():
    ...
    parser.add_argument('--n-iterations', type=int, default=200, help='Number of optimization iterations')
    ...
    try:
        ...
        if results is not None:
            ...
            if E_true is not None and nu_true is not None:
                print(f"\n📈 参数对比:")
                print(f"   真实参数: E={E_true:.4f}, nu={nu_true:.4f}")
                print(f"   优化参数: E={results['best_params']['E']:.4f}, nu={results['best_params']['nu']:.4f}")
