...
- # AGENTS.md
  ...
  **在任何时候需要项目中的上下文优先使用acemcp**

  ## 输出质量与编码准则 (Output Quality & Coding Principles)

  我生成的所有内容，特别是代码原型，都必须遵循以下准则：

  | 准则 (Guideline)     | 说明 (Notes)                                                 |
  | -------------------- | ------------------------------------------------------------ |
  | **meta.locale**      | 所有输出（代码注释、文档、报告）统一使用**已有注释的语言**（中文或英文）。 |
  | **code.security**    | **安全第一**。我生成的代码原型必须遵循安全的编码实践，规避常见的安全漏洞。 |
  | **code.reuse**       | **优先复用**。优先使用官方SDK、成熟的第三方库和项目内已有的组件，禁止不必要的自研。 |
  | **code.solid**       | **设计健壮**。设计与实现必须严格遵守 SOLID、DRY 等设计原则。 |
  | **code.style**       | **风格一致**。始终遵循目标语言的官方代码风格以及项目已有的风格。 |
  | **code.complete**    | **实现完整**。禁止提供 MVP、占位符或最小实现。原型必须是逻辑完整、可供参考的。 |
  | **code.clean**       | **代码整洁**。主动删除过时或冗余的实现，保持代码的清晰性。   |
  | **code.comments.zh** | **注释清晰**。所有必要的代码注释使用中文，清晰地描述其**意图 (Why)** 和**约束 (Constraints)**。 |
...
**在任何时候需要项目中的上下文优先使用acemcp执行语义搜索******
...
- **工具能力**：
  - `search_context`: 基于自然语言查询，对代码库执行语义搜索。
- **触发场景**：
  - 当需要基于“意图”或“自然语言描述”查找代码时（例如：“搜索认证逻辑”、“查找数据库连接池配置”）。
  - 当 `Serena` 的 `find_symbol` (符号) 或 `search_for_pattern` (正则) 无法精确定位相关代码时，作为补充搜索手段。
- **调用策略**：
  - 调用 `search_context`。
  - 服务在搜索前会自动执行增量索引，确保结果最新。
- **范围控制**：
  - `project_root_path`：必须提供项目根目录的**绝对路径**。
  - `query`：必须提供描述性的自然语言查询。
...
【MCP调用简报】
服务: <serena|context7|sequential-thinking|ddg-search|playwright|acemcp>
触发: <具体原因>
参数: <关键参数摘要>
结果: <命中数/主要来源>
状态: <成功|重试|降级>
...
- `xengym/`: Core simulator package. `render/` handles scenes and UI, `fem/` defines material models, `ezgym/` wraps robots and assets, and `main.py` backs the `xengym-demo` entry point.
- `xengym/assets/`: STL meshes and sensor resources. Keep new files small and document their provenance.
- `calibration/`: FEM post-processing utilities and calibration data helpers. Align any changes with `CALIBRATION_DATA_FORMAT.md` and `calibration_example.py`.
- `example/`: Runnable demos and data pipelines used as integration references.
- `quick_test.py` and `example/test*.py`: Lightweight smoke and regression tests.

## Build, Test, and Development Commands
- Create venv (example): `python -m venv .venv` then `source .venv/bin/activate` (or `./.venv/Scripts/activate` on Windows).
- Install in editable mode: `pip install -e .`.
- Run main demo: `xengym-demo --show-left --object-file xengym/assets/obj/circle_r4.STL`.
- Smoke tests: `python quick_test.py`.
- Integration demos: `python example/demo_main.py`, `python example/data_collection.py`.

## Coding Style & Naming Conventions
- Target Python 3.9+, four-space indentation, and standard PEP 8 naming (`snake_case` functions, `PascalCase` classes, uppercase constants).
- Prefer explicit relative imports inside `xengym`; avoid wildcard imports.
- Keep docstrings concise and in English; reserve inline comments for non-obvious math or calibration logic.

## Testing Guidelines
- Extend `quick_test.py` and `example/test_*.py` when adding new features.
- Place new regression scripts next to related examples, named `test_<feature>.py`.
- For FEM or calibration changes, record baselines under `calibration/` and describe dataset provenance.

## Commit & Pull Request Guidelines
- Use `<type>: <message>` commit messages (e.g., `feat: add diff image api`, `fix: handle empty asset path`).
- Ensure each commit builds and passes smoke tests before pushing.
- PRs should include: behavior summary, manual or automated test evidence, asset changes (with locations), linked issues/tasks, and short captures or depth-field diffs for UI or sensor output changes.

## Assets & Calibration Notes
- Store large raw captures outside the repo; commit only processed assets required for reproduction.
- Name STL files descriptively (e.g., `<shape>_<metric>.STL`) and update defaults in `xengym/render/calibScene.py` when introducing new tooling.
