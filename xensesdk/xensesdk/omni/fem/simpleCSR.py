import numpy as np
from xensesdk.ezgl.experimental import CUDA_AVAILABLE
from xensesdk import MACHINE

CUDA_AVAILABLE = False  # 关闭 CUDA 加速
...
class SimpleCSR:
    ...
    def solve(cls, A: 'SimpleCSR', P: np.ndarray, x0: np.ndarray):
        ...
        """
        if CUDA_AVAILABLE:
            return cg(A.csr_matrix, cp.array(P), x0=cp.array(x0), tol=0.001, maxiter=90)
        else:
            if MACHINE == "aarch64":
                return cg(A.csr_matrix, P, x0=x0, rtol=0.01, maxiter=50)
            else:
                return cg(A.csr_matrix, P, x0=x0, tol=0.002, maxiter=90)
