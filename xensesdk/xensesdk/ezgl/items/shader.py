...
import numpy as np
import os
from .texture import Texture2D, TextureUnitPool
...
class Shader:
    ...
    def __set_uniform(self, name, value, type:str, cnt=1):
        ...
        if type in ["bool", "int"]:
            ...
            gl.glUniform1i(gl.glGetUniformLocation(self.ID, name), np.array(value, dtype=np.int32))
        elif type == "sampler1D":
            gl.glUniform1i(gl.glGetUniformLocation(self.ID, name), np.array(value, dtype=np.int32))
        elif type == "float":
