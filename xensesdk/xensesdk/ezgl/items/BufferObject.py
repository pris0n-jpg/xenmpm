...
class MemoryBlock:
    ...
    def __init__(
        ...
        Parameters:
        - blocks : List[np.ndarray], 需要存储的数据块列表
        - dsize : List[int], 每个数据块的属性大小, 如 3 for vec3, 16 for mat4
        """
        self.block_lens = [0 if x is None else x.nbytes for x in blocks]  # 块总长度
        self.block_used = np.array(self.block_lens, dtype=np.uint32) # 块使用长度
        self.block_offsets = [0] + np.cumsum(self.block_lens).tolist()[:-1] # 块偏移量
        self.sum_lens = sum(self.block_lens) # 总长度
        self.dtype = [np.dtype('f4') if x is None else x.dtype for x in blocks] # 数据类型
...
class VBO():
    ...
    def _loadSubDatas(self, block_id: List[int], data: List[np.ndarray]):
        ...
        Parameters:
        - block_id : List[int], 缓冲区块编号列表
        - data : List[np.ndarray], 每一块缓冲区对应的数据列表
        """
        self.bind()
    ...
    def getUsedSize(self, id):
        """get size of buffer"""
        return self.blocks.block_used[id] // self.blocks.dtype[id].itemsize
