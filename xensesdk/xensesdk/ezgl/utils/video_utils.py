...
class CV2VideoWriter():
    def __init__(
        ...
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # Codec for .avi format (can use other codecs like 'MJPG', 'MP4V', etc.)
        self.out = cv2.VideoWriter(str(video_path), fourcc, 30.0, (img_shape[0], img_shape[1]))

    def write(self, img):
        # Write the frame to the video file
        if img.shape[0] != self.image_size[1] or img.shape[1] != self.image_size[0]:
