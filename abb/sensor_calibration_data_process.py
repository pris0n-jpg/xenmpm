...
DATA_DIR = os.path.join(project_root, 'collector', 'data', 'sensor_calibration_cube', 'sensor_0_soft_new')
...
def main():
    ...
    for data in all_data:
        plt.plot(np.abs(data[:, x_column]), np.abs(data[:, y_column]), label=f'File {data[0,0]}', marker='o')
    plt.xlabel(x_column)
    plt.ylabel(target_y)
    ...
    all_x = np.abs(np.concatenate([data[:, x_column] for data in all_data]))
    all_y = np.abs(np.concatenate([data[:, y_column] for data in all_data]))
    ...
    plt.figure()
    for data in all_data:
        plt.plot(np.abs(data[:, x_column]), np.abs(data[:, y_column])/data[:,column_names['contact_node_num']], label=f'File {data[0,0]}', marker='o')
    plt.xlabel(x_column)
    plt.ylabel(target_y)
    ...
    all_x = np.abs(np.concatenate([data[:, x_column] for data in all_data]))
    # all_y = np.abs(np.concatenate([data[:, y_column] for data in all_data]))
    all_average_node_force = np.abs(np.concatenate([data[:, y_column]/data[:, column_names['contact_node_num']] for data in all_data]))
