...
class ServoController():
    ...
    下压
    """
    def __init__(self, pose0, TIME_STAMP,
                 log_dir=None):
        if log_dir is None:
            log_dir = PROJ_DIR / f'collector/data/test'
        ## 机器人初始化
        # 启动并连接至机器人服务
        ...
        self.log_writer = LogWriter(self.log_dir/filename,
                                    fields=["id","x_mm", "y_mm", "z_mm",'F_x','F_y','F_z',"indent_mm", 'contact_node_num', 'ux', 'uy', 'uz', 'ux^2', 'uy^2', 'uz^2'])
    ...
    def move0(self,x,y,z,qw0,qx0,qy0,qz0,velocity):
        ...
        print("move xy_elocity: ", self.robot.get_velocity())
        # z移动
        self.robot.set_velocity(velocity, velocity)
...
def main(time_stamp, z_indent):
    log_dir = PROJ_DIR / f"collector/data/sensor_calibration/sensor_2_soft_new_x"

    # 机械臂和传感器初始化
    pose0=[559,-199.9,110,0,1,0,0]# 机器人运动到指定位置0（中立初始位置）取决于任务需要
    # pose0=[550.64,-45.3,189.5 + 20,0,1,0,0]# 机器人运动到指定位置0（中立初始位置）取决于任务需要
    # pose0 = [493.93, 20.13, 100, 0, 1, 0, 0] # 与平板接触
    sc = ServoController(pose0=pose0,TIME_STAMP=time_stamp, log_dir=log_dir)
    sc.robot.set_acceleration(0.1, 0.1)
    sc.robot.set_velocity(10, 10)
    sc.sensor.get_data()  # 传感器初始化

    sc.move_to_xyz(x=559, y=-199.9, z=94.88 + 0.3) # ATIsensor
    # sc.move_to_xyz(x=550.64, y=-45.3, z=189.5)
    time.sleep(1)
    print('down contact begin')
    sc.down_util_contact()

    sc.relative_move(z=z_indent)
    time.sleep(2)

    x_cont = sc.get_robot_xyz()[0]
    y_cont = sc.get_robot_xyz()[1]
    ...
    for i in np.arange(indent_num):
        ...
        # sc.relative_move(y=dx)
        sc.relative_move(x=dx)
        time.sleep(1)

    # sc.relative_move(y=-indent_num * dx)
    sc.relative_move(x=-indent_num * dx)
    sc.relative_move(z=-3 - z_indent)
    sc.sensor.release()
    # 停止机器人马达
    time.sleep(1)
...
if __name__ == '__main__':
    z_indent_list = [0.3, 0.5, 0.7]
    for z_indent in z_indent_list:
        for i in range(3):
            TIME_STAMP = str(datetime.now().strftime('%y_%m_%d__%H_%M_%S'))
            main(time_stamp=TIME_STAMP, z_indent=z_indent)
            time.sleep(3)
